...
private:
    ...
    Matrix<double,2,2> K,P,A,B,C; 
    Matrix<double,2,1> X,Xp,Xm,Y,V; 
    Matrix<double,2,1> U;
     ...
     */
    Matrix<double,2,2> R; // covariance of observation noise
    Matrix<double,2,2> Q; // covariance of process(algorthem) noise
...
void Kalman::intialize(double t0,
    ...
    A << 1,0, 0,1;
    B << dt,0, 0,dt;
    C << 0.5*t0*t0,0, 0,0.5*t0*t0;
    // errors coefficients matrix
    
    P << 500,0, 0,500; 
    K=Matrix<double,2,2>::Zero();
...
void Kalman::handleUpdate(double t, 
    ...
    A << 1,0, 0,1;
    B << dt,0, 0,dt;
    C << 0.5*dt*dt,0, 0,0.5*dt*dt;
    }

    /*
     * update measurement + input 
     */
    Xm<<x,y;  // fill the new measurement data
    V<<vx,vy;
     ...
     */ 
        ...
        X=Xp+K*(Y-Xp);
        // update covariance
        P=(Matrix<double,2,2>::Identity()-K)*P;
        // display result
        // clear vec 
        //vec.clear();
